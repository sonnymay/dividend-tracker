...
class TickerSnapshot:
            """Lightweight, immutable snapshot of live ticker data fetched from yfinance."""
...
def _first_number(*values: object) -> float | None:
            """Return the first value that is a positive int or float, or None if none qualify."""
            for value in values:
...
def _annual_dividend_from_history(stock: yf.Ticker) -> float:
            """Sum all dividend payouts from the trailing 365 days as a fallback annual rate."""
...
def normalize_dividend_yield_percent(
            ...
            """Normalize a raw dividend yield value to a consistent percentage.

                Yahoo Finance can return yields as a decimal (e.g. 0.0344) or as a
                    percentage (e.g. 3.44) depending on the endpoint. Values <= 1 are
                        treated as decimals and multiplied by 100; larger values are kept as-is.
                            If raw_yield is None, the yield is derived from the annual dividend and price.
                                """
...
def fetch_ticker_snapshot(ticker: str) -> TickerSnapshot:
            """Fetch live price and dividend data for a single ticker via yfinance.

                Raises ValueError if no live price can be found for the given ticker.
                    """
...
def enrich_holdings(raw_holdings: list[dict]) -> list[HoldingResponse]:
            """Enrich raw Supabase holding rows with live market data from yfinance.
                ...
                Deduplicates yfinance calls by caching snapshots per ticker.
                    """
...
def build_dashboard(holdings: list[HoldingResponse], goal: GoalResponse) -> DashboardResponse:
            """Aggregate enriched holdings and goal data into a full dashboard response."""
